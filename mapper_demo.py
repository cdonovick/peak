...
def main(n):
    _demo_pe = __import__(f'peak.demo_pes.pe{n}', fromlist=('sim', 'isa'))
    sim = _demo_pe.sim
    isa = _demo_pe.isa
    del _demo_pe
    ...
    del lib
    del context
    alu = sim.gen_alu(hwtypes.SMTBitVector.get_family(), generate_assembler)
    for mod in mods:
        if mod.name in __COREIR_MODELS:
            ...
            mappings = list(gen_mapping(
                    ...
                    1,
    #                verbose=True,
                    assembler_generator=generate_assembler
                ))
            if mappings:
