...
from . import family as peak_family
...
class IR:
    ...
    def add_peak_instruction(self, name : str, input_interface : Product, output_interface : Product, fun : tp.Callable, family=peak_family, cls_name=None):
        if cls_name is None:
            cls_name = name
        ...
        for t in it.chain(inputs.values(), outputs.values()):
            if not t in t_to_tname:
                ...
                idx +=1
        class_src = [f"@_family_closure(_family)"]
