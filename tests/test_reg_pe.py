...
@pytest.mark.parametrize('op', ASM.keys())
def test_smt(op):
    fam = family.SMTFamily()
    SMTPe, isa = sim_mod.RegPE_mappable_fc(fam)
    pe = SMTPe()
    ...
    for rs1, rs2, rd in zip(RS1, RS2, RD):
        inst = ASM[op](rs1, rs2, rd)
        asm_inst = AsmInst(inst)
        assert pe(inst, rs1_v, rs2_v, rd_init).value == GOLDS[op](rs1_v, rs2_v).value
